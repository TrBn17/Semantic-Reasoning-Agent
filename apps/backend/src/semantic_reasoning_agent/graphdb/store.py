...
class Neo4jGraphStore(GraphStore):
    ...
    def sync_published_graph(self, snapshot: PublishedOntologySnapshot) -> None:
        parameters = {
            ...
            "version": self._serialize_version(snapshot.version),
            "entities": [self._serialize_entity(entity) for entity in snapshot.entities],
            "relations": [self._serialize_relation(relation) for relation in snapshot.relations],
        }
        try:
            with self._driver.session(database=self._database) as session:
                session.execute_write(self._replace_version_snapshot, parameters)
        except Exception as exc:
            raise GraphStoreError(
    ...
    def get_graph(self, workspace_id: str) -> OntologyGraphResponse:
        ...
        return OntologyGraphResponse(
            workspace_id=workspace_id,
            version=version,
            entities=[self._deserialize_entity(entity) for entity in entity_rows],
            relations=[
