...
export function GraphView() {
  ...
  /** Deferred so typing does not synchronously rebuild Cytoscape on every keystroke. */
  const filteredNodesForCanvas = useMemo(() => {
  ...
  const filteredNodeIdsForCanvas = useMemo(
    () => new Set(filteredNodesForCanvas.map((node) => node.id)),
    [filteredNodesForCanvas],
  );
  const filteredEdgesForCanvas = useMemo(
    () =>
      edges.filter(
        (edge) => filteredNodeIdsForCanvas.has(edge.sourceId) && filteredNodeIdsForCanvas.has(edge.targetId),
      ),
    [edges, filteredNodeIdsForCanvas],
  );
  ...
  const canvasHasNodes = filteredNodesForCanvas.length > 0;
  const zoomDisabled = !canvasHasNodes;
  ...
  return (
    <div className="flex min-h-full min-w-0 flex-col bg-background">
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 gap-4 overflow-hidden p-4">
        <section className="min-h-0 flex-[1.35]">
          <Card className="flex h-full min-h-0 flex-col overflow-hidden border shadow-md">
            ...
            <CardContent className="relative flex-1 bg-muted/20 p-0 dark:bg-muted/10">
              ...
              {!isLoading && !isError && hasGraphData && !canvasHasNodes && (
                <div className="flex h-full flex-col items-center justify-center gap-4 p-10 text-center text-sm text-muted-foreground">
                  <p>{t.graph.searchNoMatches}</p>
                  <Button type="button" variant="outline" size="sm" onClick={() => setSearch("")}>
                    {t.graph.clearSearch}
                  </Button>
                </div>
              )}
              {!isLoading && !isError && canvasHasNodes && (
                <GraphCanvas
                  ref={canvasRef}
        ...
        <aside className="w-[360px] min-h-0">
