...
import { getAgentSettings, updateAgentSettings } from "@/shared/api/agents";
import {
  createAgentProfile,
  listAgentProfiles,
...
function hydrateToolDrafts(
  assignments: AgentProfileResponse["tool_assignments"],
): Record<string, boolean> {
  return Object.fromEntries(assignments.map((assignment) => [assignment.tool_name, assignment.enabled]));
}
...
export function AgentSettingsView() {
  ...
  const [profileToolDrafts, setProfileToolDrafts] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    setProfileToolDrafts(hydrateToolDrafts(selected.tool_assignments));
  ...
  const createProfileMutation = useMutation({
    mutationFn: () =>
      createAgentProfile({
        ...
        tool_assignments: tools.map((tool) => ({
  ...
  const saveProfileMutation = useMutation({
    mutationFn: (profile: AgentProfileResponse) =>
      updateAgentProfile(profile.id, {
        ...
        tool_assignments: tools.map((tool) => ({
          tool_name: tool.tool_name,
          enabled: profileToolDrafts[tool.tool_name] ?? true,
        })),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.agents.profiles(workspaceId) });
              ...
              {(profiles ?? []).map((profile) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setProfileToolDrafts(hydrateToolDrafts(profile.tool_assignments));
                  }}
            ...
            {(() => {
              ...
              return (
                <div className="space-y-5 rounded-xl border p-4 sm:p-5">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="font-medium">Tool assignments</div>
                      <div className="text-xs text-muted-foreground">
                        Explicit per-profile tool binding used by the Ask workbench.
                      </div>
                    </div>
                      ...
                      {tools.map((tool: ToolSpec) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {tool.tool_family} · {tool.risk_level}
                            </div>
                          </div>
                          <input
                            type="checkbox"
                            checked={profileToolDrafts[tool.tool_name] ?? true}
                            onChange={(event) =>
                              setProfileToolDrafts((current) => ({
                                ...current,
