...
import { getBuild } from "@/shared/api/ontology";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            · Model {build.provider ?? "default"} / {build.model ?? "workspace default"} · Created {formatDateTime(build.created_at)} · Updated{" "}
      ...
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <Card>
          <CardContent className="pt-4">
            <p className="text-xs uppercase text-muted-foreground">Extraction model</p>
            <p className="text-sm font-semibold">{build.model ?? "Workspace default"}</p>
            <p className="text-xs text-muted-foreground">{build.provider ?? "default routing"}</p>
          </CardContent>
        </Card>
      </div>
